...
| Sync engine (data-loss / TOCTOU) | internal/sync, internal/store | deep audit, race | 3,11,18 | recent (HIGH fixed pass 11: PullRemoteBatch skips a Dirty resource via store.ErrKeptLocalEdit. **Pass 18 HIGH — CONFIRMED, UNFIXED**: `store.CommitPush` (remote.go:83) treats `cur==nil` (resource deleted mid-push by the event-loop while the sync goroutine's PUT was in flight) identically to `cur==pushed`, rebuilding the deleted resource as clean; `stageResourceLocked` (mutate.go:161) then unconditionally `delete(cs.tombstones,name)`, so a user delete landing during an edit-PUT is silently lost — item resurrected clean, tombstone gone, server keeps the resource, next sync a no-op. Same for the never-pushed pushCreate variant. Repro `internal/store/commitpush_deletemidpush_test.go` (ran red). Fix direction: distinguish `cur==nil`+tombstone-exists → honor the deletion / advance the tombstone ETag, don't resurrect) |
...
| Feature-promise conformance vs main.md/CLAUDE.md | (whole app) | spec-diff | 10,13,17,18 | recent (**Pass 18 MED — CONFIRMED, UNFIXED**: main.md:340 promises a `:config` reload "re-parses the account list (picker/status bar update live)", but the reloaded list is discarded — `ConfigReload` (ui/app.go:324) carries only Sync/ColorMode/Warning, `editConfigFn` (cmd/main.go) reads `cfg.Accounts` only to match the running account and never returns refreshed names, and `applyConfigReload` (ui/command.go:195) never updates `a.accounts`/`a.activeAccount` (set once in Run). A `:config` that adds/renames an `[[account]]` leaves the new account invisible in the picker + status bar and unreachable via `:account` (flashes "unknown account") until restart. Fix direction: add Accounts/ActiveAccount fields to ConfigReload, refresh both fields + rebuild the picker in applyConfigReload. Rest of the v1.1.0 promise set held under spec-diff (teardown-rebuild GC's the old store — no cross-account pointer leak; cache carry-over via unchanged ID derivation; last-active-by-id survives rename; :config never yanks a live store). Prior: 2 MED fixed pass 13: applyMutation (edit form + recur-scoped saves) and reparentSelected H/L now route through PutIfUnchanged — the "no Locate→Put clobber sites remain" invariant is now true again. Pass 17 re-diffed the passes 14/15/16 promises (method-aware redirect policy, heal-set-mirrors-validateComponent, RDATE/EXDATE multi-value independence): the redirect and reconcile promises hold; the RDATE/EXDATE-independence promise is confirmed intact for comma-listed values; the pass-17 tz.go VALUE=PERIOD-IANA-TZID mis-zone in the same resolveDateTime path (recorded as a finding in the Timezone/DST row) is now FIXED) |
| Multi-account config parse (`[[account]]` schema, `[server]` migration rejection, validateAccounts nameless/dup, ResolveActiveAccount fallback, Account lookup, Account.ID cache-namespacing) | internal/config | fuzz | 18 | recent (first audit of the v1.1.0 multi-account TOML parse added after pass 17. **HIGH — CONFIRMED, UNFIXED**: `toml.Decode` (config.go:195) is O(depth²) on deeply nested inline tables, so a config well under the `maxConfigBytes` 4 MiB read cap (config.go:23) hangs `Load()` — and thus startup — for minutes to hours with no UI and no error. Measured on vendored BurntSushi/toml v1.6.0: 8 KB→232 ms, 16 KB→840 ms, 32 KB→3.36 s (each doubling ≈4× time); a ~1 MB sub-cap file extrapolates to >1 h. The read cap bounds *bytes*, not decode CPU, and Load has no timeout — defeating the cap's stated purpose. `TestLoadCapsReadSize` only exercises a long *comment*, giving false confidence. Threat model: local user-owned 0600 config → reachable via a corrupted / mis-edited / crafted config, not remote input. Fix direction: cap decode structurally (depth/time bound) or decode with a deadline. The account-name uniqueness / migration / ID-derivation logic itself held under adversarial input — no parse finding there) |
| Global state file (`global.json` LoadGlobal/SaveGlobal, corrupt/missing→zero, atomic temp+rename, capped read, ActiveAccountID round-trip) | internal/state | fault-injection | 18 | recent (first audit of the v1.1.0 cross-account state file. The corrupt/missing→zero fail-safe and atomic write held under injected faults — no finding; a corrupt/partial/oversized global state degrades to zero and never blocks startup. Canary ESCAPE this pass: `Save`'s 0o600 file-mode contract is untested — see below) |
| Account switch-and-rebuild loop (`runTUILoop` persist-active-id-before-open, previous-account fallback on failed switch-open, fatal initial-open, unknown-target clean quit) | cmd/lazyplanner | fault-injection | 18 | recent (first audit of the v1.1.0 switch state machine. Injected store.Open failures exercise the fallback (reopen previous working account) and second-failure-fatal logic; the documented persist-before-open crash-window residual (sub-ms) is accepted. No finding on the loop itself. Canary ESCAPE: the `components()` --tasks VTODO/VEVENT helper in the sibling `calendar.go` is untested — see below) |
| `:account` command + picker (switchAccount case-insensitive validation, already-active no-op, unknown flash, requestSwitch/RunResult.SwitchAccount, no-accounts path) | internal/ui | input-edge | 18 | recent (first input-edge of the v1.1.0 command handler; picker draw was stress-covered by TestAccountPickerStress. Adversarial names + switch-while-modal states surfaced no *new* command-handler defect, but the input-edge exposed the `:config`-reload account-list staleness recorded in the feature-promise row above — MED, CONFIRMED, UNFIXED) |
...
- **Sync core deep concurrency / TOCTOU** — re-swept pass 18 (first deep TOCTOU re-visit since pass
  ...
  fix is not yet landed, and the deeper reconcile-vs-concurrent-pull matrix (tombstone/keep-both
  races beyond the CommitPush window) is still only shallowly covered — the surface is warm again but
  not cleared.
- **Multi-account config parse — startup-hang via O(depth²) TOML decode** (pass-18 HIGH, UNFIXED) —
  a deeply nested inline-table config well under the 4 MiB read cap hangs `Load()`/startup for
  minutes-to-hours (`config.go:195`, no decode timeout). Repro reproduced (32 KB→3.36 s, quadratic).
  The `maxConfigBytes` cap bounds bytes, not decode CPU. Reachable only from a corrupted / crafted
  local 0600 config, but a full silent startup hang. See the multi-account config-parse row.
- **`:config` reload does not re-parse the account list live** (pass-18 MED, UNFIXED) — contradicts
  main.md:340; a `:config`-added/renamed account stays invisible and unreachable until restart
  (`ConfigReload` carries no account list; `applyConfigReload` never refreshes `a.accounts`). See the
  feature-promise row.
- **v1.1.0 permission-mode / component-set canary holes** (pass-18, OPEN) — the config
  group-readable warning mask, the state-file 0o600 mode, the CLI `components()` --tasks/--both
  helper, and `treeNodeAtY`'s upper bound are all unguarded (see the pass-18 canary section); a
  regression on any ships silently.
...
## Escaped mutation canaries — pass 18 (4 of 4 escaped → OPEN)

All four canaries this pass escaped — each a test-coverage hole on a v1.1.0 / sibling surface (code
correct today, but a plausible regression on that exact path would ship silently). None fixed this
pass; each needs a boundary test.
...
- **ESCAPE** — `internal/config/config.go` `permissionWarning()`: narrowing the loose-permission
  ...
  ./internal/config/`. The only permission test, `TestLoadWarnsOnLoosePermissions`, uses mode 0o644,
  which still trips the narrowed 0o007 mask via the other-readable bit — so a **group-readable-only**
  config (0o640) silently stops warning "may hold a password — chmod 600 it" and no test notices.
  Suggested guard: table the warning across 0o600/0o640/0o604/0o644.
- **ESCAPE** — `internal/state/state.go` `Save()`: flipping the state-file mode 0o600 → 0o644
  (world-readable) passed `go test ./internal/state/`. No test asserts the written file's FileMode
  (round-trip/temp-file/read-cap/JSON-error tests only) — the 0o600 permission contract (and the
  0o700 MkdirAll dir mode) is unguarded, so a regression loosening state-file permissions ships
  silently. Suggested guard: `os.Stat`/`FileMode()` assertion after Save.
- **ESCAPE** — `cmd/lazyplanner/calendar.go` `components()`: flipping the `--tasks` branch
  ...
  `slugify()`, and `joinWarnings()` have zero test coverage — the CalDAV MKCALENDAR
  supported-component-set that decides whether a created collection holds events or tasks is
  unguarded, violating the events↔VEVENT / tasks↔VTODO invariant silently. (The credential guard in
  conn.go IS pinned by the pass-16 `TestConnFlagsClientRequiresAllCredentials`.) Suggested guard:
  table `components(tasks,both)` over --tasks / --both / neither.
- **ESCAPE** — `internal/ui/mouse.go` `treeNodeAtY` (~line 101): off-by-one `if idx >=
  len(visible)` → `if idx > len(visible)` passed `go test ./internal/ui/`. A double-click on the row
  exactly one past the last visible tree node would index `visible[len]` → panic the whole TUI. The
  double-click tests hit valid in-range rows only; the failing boundary (`idx==len(visible)`) is
  never exercised. Suggested guard: double-click the row one past the last node, assert no panic /
  nil target.
