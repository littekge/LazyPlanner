...
- **`moveSubtreeOps` dest-Put orphan-retry residual** (pre-existing, not introduced this pass) — OPEN,
  named blind spot: `internal/ui/yankpaste.go:344`'s destination-side write is a fresh `store.Put` (a
  legitimate create — the moved item has no prior resource at the destination name) wrapped in a
  swallowed-error rollback; if a first attempt's dest-side `Put` partially lands (e.g. succeeds but the
  caller treats it as failed / a subsequent step errors) and the caller retries the whole move, the retry's
  fresh-create bare `Put` at the same destination name can silently clobber the orphaned copy the failed
  first attempt already wrote there — a bare-Put site that is *correctly* a create on the happy path but
  not proven safe across a retry. Not a pass-19 finding (no repro was run against it this pass); flagged
  here so the next pass that touches yankpaste.go/moveSubtreeOps treats it as an open question rather than
