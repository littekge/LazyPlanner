...
- **Recurrence expansion has no aggregate/render-path budget** (pass-21 MED) — CONFIRMED, UNFIXED
  (repro-verified RED, **test LEFT IN TREE and currently failing**). The scale guardrail bounds a
...
- **PROPPATCH 207 treated as success without per-property status** (pass-21 MED) — CONFIRMED, UNFIXED
  (repro-verified RED). `SetCalendarProps` (proppatch.go:48) returns nil on any 207 Multi-Status
...
## Escaped mutation canaries — pass 21 (1 of 4 genuinely escaped → OPEN)
...
- **ESCAPE (OPEN, verified)** — `internal/caldav/object.go` `PutObject` (line 87): removing
  `http.StatusOK` from the accepted-success set (`!= StatusCreated && != StatusNoContent && !=
  StatusOK` → drop the `StatusOK` clause) passed `go test ./internal/caldav/` (re-verified this
  synthesis: mutation applied, suite still `ok`). A server that answers a PUT with **200 OK**
  (RFC-legal and returned by some real CalDAV servers) would be treated as a write failure — surfacing
  a spurious error and leaving the edit marked dirty/conflicted though the write succeeded. The line's
  own comment documents 200 as valid, but the success-path tests cover only 201 (`TestPutObjectCreate`)
  and 204 (`TestPutObjectUpdateSendsQuotedIfMatch`); no test returns 200 on a PUT. Close with a test
  issuing a PUT that responds 200 OK and asserting `PutObject` returns `(etag, nil)`.
