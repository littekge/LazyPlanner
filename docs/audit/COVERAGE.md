...
- **SELECT bulk-delete drags co-resident bystanders** (pass-19 HIGH) — OPEN/UNFIXED: `bulkDelete` and
  `deleteWholeObject` delete the whole bundled `.ics` per UID instead of isolating the selected
  component, so a never-selected top-level sibling in the same resource is silently deleted and
  server-DELETE'd. Repro `internal/ui/coresident_delete_test.go`. Fix: mirror the move path's
  `model.RemoveComponent` rewrite. Reachable only from a bundled/foreign/migrated resource (LazyPlanner
  writes one item per resource), but permanent silent loss when it lands.
- **ReanchoredRecurrence "last <weekday>" / positive-nth day-move** (pass-19 HIGH + MED) — OPEN/UNFIXED:
  a backward move onto a weekday whose true last occurrence is later emits `BYDAY=-1<wd>` contradicting
  the moved DTSTART (vanishing instance — the exact bug the function guards against); a forward move
  into a 5th week emits out-of-vocabulary `BYDAY=5<wd>`. Repros
  `internal/model/reanchor_lastweekday_repro_test.go` (+ the positive-nth repro). Fix: block the move
  when the re-derived nth isn't in {1,2,3,4,-1} or doesn't match newStart.
- **pushDelete 412 resurrect clobbers a concurrent undo/re-create** (pass-19 HIGH) — OPEN/UNFIXED: the
  delete-conflict twin of the pass-18 CommitPush `cur==nil` HIGH; the 412 branch's bare unconditional
  `PutRemote` overwrites a resource RestoreDirty re-created mid-DELETE and stashes only the server
  version, so the undone content is unrecoverable. Repro `internal/sync/tombstone412_undo_race_test.go`.
  Fix: resurrect only when the resource is still absent (expectedPrev guard).
- **Undo of a co-resident multi-root move loses a root** (pass-19 HIGH) + **reparentTo bare Put**
  (pass-19 MED) — OPEN/UNFIXED: stacked same-resource undo ops replayed in append order clobber the
  full-bundle Restore; and the single-item same-list reparent is the one write path still on a bare
  Locate→Put (the pass-13 "no clobber sites remain" claim is again FALSE for this path). Repros
  `internal/ui/undo_multiroot_clobber_test.go`, `internal/ui/reparent_clobber_test.go`.
- **parseEveryRecur impossible month-day** (pass-19 LOW) + **SELECT vim-count leak** (pass-19 LOW) —
  OPEN/UNFIXED: "every feb 30" is consumed+discarded and misanchors a yearly series; a vim count typed
  in SELECT leaks past a bulk-op key into the next motion. Repros
  `internal/ui/countleak_repro_test.go` (count) + the parseEveryRecur repro.
...
## Escaped mutation canaries — pass 19 (2 of 3 escaped → OPEN)

Both escapes are test-coverage holes on this pass's target surfaces (code correct today; a plausible
regression on that exact path would ship silently). Neither is closed yet — close each with a
boundary test verified RED under its mutation when the pass-19 findings are fixed.
...
- **ESCAPE** — `internal/ui/selection.go` `dayInRange()` upper-boundary flip
  ...
  timegridview.go:575) that style in-range SELECT days; neither appears in any `*_test.go`. The
  SELECT-range *materialization* (daysRange/selRange) is well covered (TestDaysRange*), but the
  *visual highlight* boundary it drives is unguarded — an off-by-one making the highlighted band stop
  one day short of the acted-on interval (last/cursor day not shown selected though bulk ops still hit
  it) ships undetected. Guard: assert the cursor/`to` day is in-range at the band's upper edge.
- **ESCAPE** — `internal/model/quickadd.go` `parsePriority` (line 434): narrowing the numeric
  upper bound `n <= 9` → `n <= 8` passed `go test ./internal/model/`. Quick-add priority tests
  (quickadd_test.go) exercise only `!high` and `!1`; no test parses a numeric `!9`/`!8`, so the 1–9
  edge is entirely unguarded (priorityrange_test.go covers the separate iCal-clamp path; the fuzz
  only asserts 0–9, which a narrowed bound never violates). Guard: table `!9`/`!8`/`!10` through
  parsePriority.
