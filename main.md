...
**Not yet audited (next):** the Raspberry Pi target on real hardware (on-device timing/kiosk — needs a physical Pi) remains the sole never-covered product surface. The pass-11/12 stale surfaces (grab-mode, recurrence-edit UI, sync concurrency/CTag/background goroutines, undo stack, quick-field/completion write paths, color/privilege PROPFIND decode) are now **recent**, and the two recurring data-loss classes — `Locate→Put` no-version-check and `Restore`-replays-clean-and-stale — are structurally closed. The natural next target is a **whole-app spec-diff** re-run against the invariants the recent passes introduced (`PutIfUnchanged` routing, `RestoreDirty`, `DetachTodoOccurrence`, the sibling rollbacks), which pass 12 explicitly did *not* re-cover. Scale correctness is covered by benchmarks; on-hardware numbers remain to be measured. Remaining known gaps: a UID-less component is display-only (a fabricated UID would churn sync identity), and a couple of very-low-reachability go-ical encoder constraints (empty VCALENDAR, VTIMEZONE-needs-a-child) are still not auto-healed. Full `sync-collection` incremental sync is a deliberate feature deferral, not a bug.
