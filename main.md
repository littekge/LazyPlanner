...
**The overview column** (left) holds three small panels: **Calendars** (the calendar list, with visibility toggles), **Tasks** (the **task lists** — calendars that support todos, incl. empty ones, *not* the full tree), and **Agenda** (today's events + due tasks). `c`/`t`/`a` **focus the corresponding overview panel** — the highlight lives there and moves through its rows — while the Main pane shows the matching view. `Enter` dives from the overview into the Main pane where that makes sense; `Esc` backs out to the overview. Each Calendars row shows a `●` bullet in the calendar's server color (a **hidden** calendar drops the bullet, so its off-the-views state reads at a glance next to the `(hidden)` marker) and a component-set tag — `[events]`/`[tasks]`/`[both]`, `[?]` when unknown, `[ro]` when read-only.
...
Each create has a **quick-add** form (a one-line smart-parsed input) and a **full form**, reached by distinct keys folded into the chorded keymap (the `i` prefix; Shift = the full form). The quick-add parser is deliberately conservative — a token is claimed only when it clearly matches a documented form, and everything unmatched becomes the title. Tokens are **whitespace-delimited** (a sigil matches only at a token's start, so `bob@example.com` and `task!5` are inert title text) and each slot is **first-match-wins**:
...
`V` enters **SELECT**, a vim-style multi-select layer reachable from the task tree, the un-drilled calendar (a day range), or a drilled day's item list. The range is a **contiguous anchor→cursor span** — anchored where `V` was pressed, extended by ordinary cursor motion in whichever of those three contexts is active — never a scattered multi-pick. The range is **derived, not stored**: `selRange()` re-slices the anchor and the view's current cursor position on demand, so it can never drift from what is actually on screen, and it revalidates on every refresh. A **lost** anchor (a remote delete, an emptied drilled day) exits SELECT with a flash rather than acting on a guess — but a day-range anchor can't itself vanish, so an anchored range spanning only empty days is still a **valid** selection, extended (`f`/`b`) toward a day that has items. SELECT is modal like GRAB: it swallows context-switch, edit, and even **modified** motion keys (Ctrl-arrows can't sneak a pane resize in mid-select), and the badge shows `SELECT`. One bulk action — `Space` complete, `d` delete, `y`/`Y` yank/copy (tree only), or `m` grab — applies to the whole range as a single all-or-nothing operation with **one compound `u` undo step**: each first filters out items it can't act on (recurring events, read-only, missing, already-done, folders with open subtasks, undated) — delete/yank's subtree-absorption dedupe runs only against what survives that filter, so the confirm/summary count always matches what's actually acted on — then reports the skip counts in the flash. Bulk grab reuses grab's per-nudge mechanics but applies **one uniform date-shift** to every selected item at once, returning to SELECT (not exiting it) on `Esc` so a grab can be retried. Modes nest the same way GRAB does under DRILL: `DRILL → SELECT → GRAB`, each layer's `Esc` unwinding exactly one level.
...
**Convergence is measured as a severity-weighted trend, not a raw finding count** (the explicit criteria live in `docs/audit/PROTOCOL.md` — "Convergence — the stop rule"). The trend is genuine but not yet converged: HIGH ran 5 (pass 10) → 1 (pass 13) → 0 (pass 14) → 2 (pass 15) → 2 (pass 16) → **0 (pass 17)**. Pass 17 was the first **re-sweep** pass, and it half-confirms the closure: HIGH returned to zero (the covered surfaces held up under a fresh method-diverse look — no HIGH silent-data-loss/brick bug), but it still surfaced 2 MED (both a *missing-guard-that-a-sibling-has* shape) and, more tellingly, **all four mutation canaries escaped** — the worst canary result in the record, two of them *twins of already-known patterns* (the `DayAgenda` upper-bound twin of the pass-14 lower-bound escape; the `reconcileReadOnly` twin of the covered read-write guard). Status against the criteria: (1) **criterion 1 (headless matrix covered once) is met**; (2) **criterion 2 (two consecutive no-HIGH passes) now sits at one** — pass 17 is the first, so one more clean re-sweep earns the streak; (3) pass 17 added no *new* root-cause class (both MED are sibling-parity gaps, now fixed). The standing weak spot is the **test net, not the code**: the recurring twin-boundary / sibling-guard-parity canary escapes say the regression net is being extended point-by-point rather than by boundary-class — codified as a testing guardrail in `docs/audit/PROTOCOL.md` (test *both* sides of every half-open window; when a guard exists on one path, mirror its canary onto every sibling path). Estimated **~1 more re-sweep** pass to earn the two-consecutive-no-HIGH streak, with the sync-core deep concurrency/TOCTOU (last deep audit pass 11) the main heavier surface still to re-examine. Permanently-accepted gaps: the **Raspberry Pi on real hardware** (unauditable headlessly); a missing **UID** on any component is not fabricated (churns sync identity), so a UID-less component stays display-only and a resource mixing it with a valid sibling drops the whole resource on import (pass-15 MED accepted residual); the empty-VCALENDAR encoder constraint is not auto-healed; and the center agenda-board has no click-to-select (pass-16 UI follow-up).
