...
- **Pass 24** (2026-07-26) — **cancelled mid-run** and therefore not a convergence data point: launched unbounded, it exhausted the session budget before its verify, canary and synthesis stages, so no adversarial verification and no mutation canaries ran. The remaining budget went on fixing what it had already found. **5 defects reproduced (HIGH 1 · MED 4), 4 fixed**, each with a repro executed before and after the fix and mutation-checked by hand. The HIGH was its first look at `internal/caldav` write paths — a surface flagged unswept for two prior passes: a protocol-relative href (`//evil.host/x.ics`) replaced the endpoint's authority in `Client.resolve`, shipping the Basic-auth app password and the whole calendar body to a foreign host while reporting success. The MEDs: a tombstone that never converged when the resource was already gone server-side (wedging the delete and disabling the CTag short-circuit forever), `sd` failing to re-anchor a recurring todo's day-pinning rule (an existing guardrail's sweep list missing a door, not a new class), and a task's LOCATION erased by both a quick-set and a no-op form save. The fifth is an open pre-existing residual (a timed recurring todo baking a DST gap into its anchor — structural, see `COVERAGE.md` item 13). Notably the Plan phase ranked the *same-day* wall-clock/zone work as `never`-audited and made it target #1, so a fresh-code blind spot did not materialize. `more_passes_recommended`; full report in `docs/audit/passes/PASS-24.md`.
...
- **Release readiness**: all three scope phases are complete, the TZID arc's own two regressions are
  fixed, and pass 24's four fixable defects — including a HIGH credential-leak in `internal/caldav` —
  are fixed (below). Nothing scoped for v1.5.0 remains open; the one open finding
  (`COVERAGE.md` item 13) is a pre-existing structural residual, not a regression. The two `never`-state coverage rows are both
...
**A server href may never move a request off the configured endpoint's origin.** Every href LazyPlanner receives is untrusted server input, and RFC 3986 reads a leading `//` as the start of an authority — so a multistatus href of `//other.host/x.ics` is a protocol-relative URL, not a path, and resolving it against the endpoint replaces the host. `internal/caldav`'s `Client.resolve` (the single chokepoint all ten request builders pass through) therefore rejects any reference resolving to a different scheme+host, and `DownloadAll` drops such an href at ingest so it is never persisted as a `Resource.Href`. Both layers are needed: the resolve check also covers hrefs already poisoned in a sidecar by an earlier sync, which the ingest drop cannot reach.

- Origins compare on scheme and host with default ports folded, so an absolute same-origin URL — the form many servers legitimately return — still works.
- **Accepted cost**: a deployment whose server advertises hrefs on a *different* origin than the configured `url` (a reverse proxy returning its public name) now errors instead of following them. That is the correct default for a request carrying the account's app password; the error names both origins, so the fix is to point `url` at the advertised name.
