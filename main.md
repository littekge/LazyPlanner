...
**Convergence is measured as a severity-weighted trend, not a raw finding count** (the explicit criteria live in `docs/audit/PROTOCOL.md` — "Convergence — the stop rule"). The trend is mixed and honest: HIGH ran 5 (pass 10) → 1 (pass 13) → 0 (pass 14) → 2 (pass 15) → **2 (pass 16)**. Passes 15–16 were the gap-closing sweeps main.md had called for, and they proved the point — the never/stale cells (CalDAV-write, store-load) and the *incompletely-closed* encoder-heal class still harbored HIGH silent-data-loss/brick bugs, not just a MED/LOW tail. Status against the criteria: (1) **criterion 1 (headless matrix covered once) is now effectively met** — mouse/`:config` was the last stale cell — but pass 16 showed "covered once" isn't sufficient when a prior closure was *incomplete*, so a re-diff against go-ical's encoder rules is now a standing guardrail; (2) **criterion 2 (two consecutive no-HIGH passes) sits at zero** — passes 15 and 16 each found 2 HIGH; (3) pass 16 *reopened* a class rather than adding a new one (now guarded). Estimated ~1–2 **re-sweep** passes to earn a clean no-HIGH streak (the surfaces are covered; the need now is confirmation, not new ground). Permanently-accepted gaps: the **Raspberry Pi on real hardware** (unauditable headlessly); a missing **UID** on any component is not fabricated (churns sync identity), so a UID-less component stays display-only and a resource mixing it with a valid sibling drops the whole resource on import (pass-15 MED accepted residual); the empty-VCALENDAR encoder constraint is not auto-healed; and the center agenda-board has no click-to-select (pass-16 UI follow-up).
