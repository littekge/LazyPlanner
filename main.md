...
**Convergence is measured as a severity-weighted trend, not a raw finding count** (the explicit criteria live in `docs/audit/PROTOCOL.md` — "Convergence — the stop rule"). By that yardstick the trend is healthy: HIGH severity fell 5 (pass 10) → 1 (pass 13) → **0** (pass 14), even as total count held ~5–7; the bugs now need increasingly exotic triggers (a foreign import with a multi-valued RDATE, a degraded network mid-delete). But the phase is **not yet converged** against the criteria: (1) the headless surface×method matrix is not yet covered once — **CalDAV response-parse fault-injection** (stale since pass 7), **store write-pipeline disk-fault atomicity** (pass 9/10), the full **reconcile keep-both/`Forget`/read-only-twin** cells, and **mouse/`:config`** remain never-or-stale; (2) only pass 14 has hit no-HIGH (the criterion needs two consecutive); (3) pass 14 introduced a new root-cause class (RDATE/EXDATE semantics, now guarded). Estimated ~2–3 focused passes out — bottlenecked on closing the remaining matrix cells and landing a second consecutive no-HIGH pass, not on the code being fragile. Permanently-accepted gaps: the **Raspberry Pi on real hardware** (unauditable headlessly), a UID-less component is display-only (a fabricated UID would churn sync identity), and two very-low-reachability go-ical encoder constraints (empty VCALENDAR, childless VTIMEZONE) are not auto-healed.
