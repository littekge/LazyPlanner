...
Layout proportions adapt automatically to terminal size (tview reflows the `Flex` tree on every resize). On top of that, two interactive controls (added in build step 10):
...
Pass 6 (2026-07-13): a **terminal/display robustness stress pass** over the six custom-drawn widgets (the layer that produced the two historical freeze bugs). Display-hostile content (over-long/CJK/emoji/combining/RTL/control-char titles, 150 same-day events, deep+wide task trees) is drawn through every mode/view/drill state across geometries from 1×1 to 400×150, with panic-recover + watchdog, and the grids' drill/scroll paths driven to the far index at 1–3-row heights. **No panic or hang was found** — the widgets handle rune-width/clipping/scroll boundaries correctly; the stress tests (`internal/ui/displaystress_test.go`) stay as permanent regression guards on the normal gate. See `log.md`.

Pass 7 (2026-07-13): a **network fault-injection pass** hardening the CalDAV trust boundary against a hostile/buggy server. Fixed an unbounded response-body read (a `cappingTransport` now caps every body at 64 MiB, covering go-webdav's reads and our own PROPFINDs, so a server streaming an unbounded body can't hang a sync or exhaust memory — a real Pi risk). Tests cover oversized/streaming, malformed, truncated, wrong-status, and deeply-nested responses (all degrade cleanly, no panic/hang), plus sync-level faults (discovery failure = clean error; transient push failure keeps the edit dirty and recovers). See `log.md`.

Pass 8 (2026-07-13): an **exhaustive timezone/DST recurrence sweep** (`internal/model/tzsweep_test.go`) — wall-clock preservation across both DST directions (incl. southern hemisphere and half-hour offsets), leap-day yearly, monthly-on-the-31st, year boundaries, UTC/floating/Windows-zone, all-day across DST, and the spring-forward-gap / fall-back-ambiguous edges. **No bug found** — behavior is correct; the sweep stays as permanent regression guards. See `log.md`.

Pass 9 (2026-07-13): a **pre-1.0 "final unhardened-areas" audit** (four parallel read-only audits over the surfaces earlier passes didn't cover: local-disk/config input boundaries, store filesystem robustness, the UI input-handler panic surface, and the model write-side mutation primitives). Sixteen findings fixed, each adversarially verified, with a regression test and its own commit. **HIGH:** a path-traversal calendar id (`..`) that could `RemoveAll` the whole account dir; an unguarded write-side recurrence expansion that crashed on a degenerate rule; and three recurrence-edit data-loss/iron-rule bugs (dropped VALARMs on override + split, dropped future RECURRENCE-ID overrides on split). **MED:** malformed-config and unreadable-cache-root startup handling, a swallowed revert-write divergence, an all-day `UNTIL` value-type violation, a leaked-orphan calendar create, and `password_command` `WaitDelay`/empty-output. **LOW:** name-length cap, temp-file sweep, RDATE-past-COUNT=1 advance, two UI recurrence-edit robustness fixes, CLI unknown-command/help/version, and local-read size caps. Added a **recurrence-mutation fuzz target** (`FuzzRecurrenceMutations`) guarding the whole write-side class. The UI input surface and the sidecar-corruption path were audited and found already sound. See `log.md`.

Pass 10 (2026-07-13): the **first run of the `hardening-audit` workflow** (`.claude/workflows/hardening-audit.js`, run via `/audit`), targeting the surfaces the coverage ledger (`docs/audit/COVERAGE.md`) still marked *stale* after pass 9. 63 agents surfaced **9 findings (5 HIGH, 4 MED), each confirmed with an executed repro**, plus 3 mutation-canary test-coverage holes — all now fixed with regression tests. **HIGH:** five decode-but-unencodable go-ical shapes (DTEND/DUE+DURATION, DURATION-without-DTSTART, empty VTIMEZONE, VJOURNAL/VFREEBUSY nesting) each poisoning a whole resource on re-encode, and yank/paste cross-list move dragging co-resident todos out of a bundled `.ics` (all reachable only via a foreign/bundled resource — the app never writes these shapes itself). **MED:** `:config` failing a flag-bearing `$EDITOR`; a crash between the `.ics` and sidecar renames stranding an offline edit; a copy duplicating bundled siblings with original UIDs. The audit **did not converge** (a fresh method on stale surfaces still found HIGHs), and the workflow's own enforcement/verification caught a false "committed" claim in its summary — reinforcing that readiness is a trend across diverse passes, not a single clean verdict. See `docs/audit/passes/PASS-10.md` and `log.md`.

Pass 11 (2026-07-15): a **stale/never-surface sweep via the workflow** over the grab-mode temporal-manipulation state machine, the recurrence-edit UI orchestration, and the sync concurrency / CTag / background-goroutine machinery. 51 agents surfaced **7 findings (3 HIGH, 2 MED, 2 LOW), each with an executed repro**, plus 2 mutation-canary holes — all fixed. The three HIGH shared one root cause — **a multi-write op with no rollback on a later-write failure** — while the sibling `beginGrabFuture` already had the guard: `PullRemoteBatch` clobbering a concurrent local edit to a href-less pull-orphan (now skips a Dirty resource via `store.ErrKeptLocalEdit`), and `commitSplit`/`commitDetach` leaving a half-completed series (now roll back). MED/LOW: `cancelGrab` swallowing revert errors, a detached recurring-todo dropping unmodeled props, and the grab nudge's `Locate→Put` clobbering a concurrent pull (new `store.PutIfUnchanged`). See `docs/audit/passes/PASS-11.md` and `log.md`.

Pass 12 (2026-07-15): the workflow took the **pass-11 named follow-up — the systemic `Locate→Put` no-version-check pattern outside grab** — head-on, plus the session undo stack and the calendar color/name PROPFIND decode. 48 agents → **7 findings (2 HIGH, 5 MED), each repro'd**, + 3 canary holes, all fixed. It **confirmed and closed two recurring root-cause classes**: (1) `Locate→Put` clobber — quick-field `sp`/`sd`, `Space` completion, and recurring-todo advance now all route through `store.PutIfUnchanged` (grab was fixed in pass 11); (2) `Restore` replays clean+stale — undo of a *synced* delete silently `Forget`-dropped the resurrected item and undo of a synced edit was pulled back, both fixed with `store.RestoreDirty` (mark the resurrection/revert Dirty so sync pushes it or keeps-both). Also a HIGH iron-rule breach where a quick `sp`/`sd` **flattened a foreign IN-PROCESS/CANCELLED task's STATUS + dropped PERCENT-COMPLETE** (and restamped COMPLETED) — `EditTodo` now only rewrites the completion trio when completed-ness actually changes; and a raw-`<href>` keying bug that dropped server colors **and made read-only detection fail open** (now a shared `caldav.hrefKey` decodes the href like go-webdav). See `docs/audit/passes/PASS-12.md` and `log.md`.

Pass 13 (2026-07-16): the **whole-app spec-diff re-run** pass 12 named as the next target — a spec-diff against `main.md`/`CLAUDE.md` focused on the invariants passes 11/12 introduced (`PutIfUnchanged` routing, `RestoreDirty`, `DetachTodoOccurrence`, the sibling rollbacks), paired with two never/stale surfaces: the **sync reconcile state-machine** case matrix (data-loss) and the **CalDAV request-construction side** (MKCALENDAR/PROPPATCH/DELETE + idempotency). 44 agents → **5 findings (1 HIGH, 4 MED), each with an executed failing-test repro**, + 4 escaped mutation-canary holes — all now fixed. **The spec-diff disproved pass 12's "the `Locate→Put` class is structurally closed" claim** — the spot check had missed two sites (MED): `applyMutation` (the edit form + all recurrence-scoped saves — the app's *longest* clobber window) and `reparentSelected` (`H`/`L`), both now routed through `store.PutIfUnchanged`. **HIGH:** a degraded download (bulk REPORT fails → per-resource GET fallback) inferred an unfetchable-but-still-listed resource as a **remote deletion** — Forgetting a clean local item / raising a false `ServerDeleted` conflict; `downloadResilient` now returns the listed-but-unfetched set and both reconcile paths skip it. **MED:** `DELETE` (404/410) and `MKCALENDAR` (405 after a lost 201) were non-idempotent, wedging a calendar pending-delete/pending-create forever — both now treat already-gone/already-exists as success. The four canary holes (LayoutDay touching-boundary lane-minimality, CTag-not-cached-after-skip, `DeleteObject` empty-ETag `If-Match:*`, config read size cap) got boundary tests. The pass **did not converge** (a fresh method on stale surfaces reopened a class thought closed and found a new data-loss cell), reinforcing that "closed" needs an exhaustive sweep, not a spot check. See `docs/audit/passes/PASS-13.md` and `log.md`.

**Audit tooling (pass-10 onward):** hardening passes now run through the reusable `hardening-audit` workflow — coverage-first surface selection, method-diverse fan-out, adversarial verification with runnable repros, mutation canaries that test the suite, and a report that reports bounded *residual risk* (never "clean"). The living coverage ledger and per-pass reports live under `docs/audit/`; see `docs/audit/PROTOCOL.md`.

**Not yet audited (next):** the Raspberry Pi target on real hardware (on-device timing/kiosk — needs a physical Pi) remains the sole never-covered product surface. Pass 13 ran the whole-app spec-diff and — after an exhaustive site sweep — the `Locate→Put` no-version-check class is closed *again* (`applyMutation` + `reparentSelected` were the two missed sites), so **all UI write paths now route through `store.PutIfUnchanged`**; the CalDAV request side (MKCALENDAR/DELETE) is now idempotent. The sync **reconcile case matrix** is only *partially* enumerated — pass 13 found and fixed the degraded-download cell, but the full local×server matrix (all keep-both / `Forget` branches) is not exhaustively covered and is the natural next data-loss target. Also still stale-but-untouched: **timezone/DST** (pass 8), **UI draw widgets** (pass 6), and the bulk of **non-command key/chord input-edge** handlers (only the `:` command dispatch was covered in pass 13). Scale correctness is covered by benchmarks; on-hardware numbers remain to be measured. Remaining known gaps: a UID-less component is display-only (a fabricated UID would churn sync identity), and a couple of very-low-reachability go-ical encoder constraints (empty VCALENDAR, VTIMEZONE-needs-a-child) are still not auto-healed. Full `sync-collection` incremental sync is a deliberate feature deferral, not a bug. **The CalDAV/NextCloud test server is offline as of 2026-07-16**, so the opt-in live suite can't be re-run — sync findings are verified headlessly until it returns.
