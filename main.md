...
**Status (2026-07-24)**: step 0 and both gap-closers are **shipped** (subagent-driven build off the plan `docs/superpowers/plans/2026-07-24-v1.5.0-step0-and-gap-closers.md`; whole-branch review clean). **Phase 1 is complete**: 962 claims verified (`docs/audit/specdiff/CLAIMS.md` — the living inventory), 21 adversarially-confirmed divergences owner-triaged and fixed (all fix-doc), ~20 reverse-sweep doc gaps documented, plus an owner-approved four-fix polish bundle (`sp` warning relay, stale-rect double-click guards on the agenda board and tree, `j`/`k` in the Conflicts list + account picker, Ctrl-W cancel restores the accordion) — every code fix repro-first with a regression test. **Phase 2 is also complete**: an exhaustive key×context matrix (`docs/audit/specdiff/MATRIX.md`, 529 verified cells) found 20 divergences, all owner-triaged and fixed (2 UI-text bugs, bulk-grab axis alignment, undo-drill preservation, `J`/`K` todo flash, `q`-close on the account/color pickers, a mode-adaptive hint bar, and ~10 README/`:help` doc corrections), plus a `j`/`k`→`motionArrow` refactor folding duplicate key-mapping onto the existing idiom; whole-branch review clean. The matrix was subsequently extended with the `:` command surface (35 rows) and mouse-contract axes (29 rows), expanding the verified-cell count to 593; its 4 additional divergences were owner-triaged and fixed. **Phase 3 (deep audit) has its required minimum one pass done**: Pass 19 (`docs/audit/passes/PASS-19.md`) targeted the never-audited v1.4.0 SELECT/bulk-ops surface, the v1.3.0 recurrence-rewrite primitives, the v1.2.0 quick-add grammar additions, and the reconcile-vs-concurrent-pull matrix beyond the CommitPush window — 8 confirmed findings (4 HIGH, 2 MED, 2 LOW), all fixed repro-first with regression tests, plus both escaped mutation canaries closed; a final whole-arc review came back READY TO MERGE (0 Critical/Important, 7/7 cross-cutting checks). The review's own recommendation was `more_passes_recommended` — pass 19's severity trend ran up, not down (HIGH 2→4, total 3→8) — so hardening continues in future v1.5.x passes post-release; this release gate item is satisfied by the one completed, fully-resolved pass. **Pass 20** (2026-07-25, `docs/audit/passes/PASS-20.md`) followed on the pass-19-flagged next targets (the reconcile matrix beyond CommitPush/pushDelete-412, `moveSubtreeOps`'s cross-collection write, bulk-grab, and a whole-app promise spec-diff) — 5 confirmed findings (1 HIGH, 2 MED, 2 LOW), all fixed repro-first, plus both escaped mutation canaries closed. The HIGH (reconcile step-(A) `Forget` clobbering a concurrent edit) was the **third reopening of the "resource-is-gone" reconcile class** — now guarded by `store.ForgetIfUnchanged` and codified as a dedicated Hard-won guardrail. Raw severity trended down (HIGH 4→1, total 8→5) but the class reopening plus 2/4 canary escapes keep it `more_passes_recommended`; the named residual is the cross-package resource-is-gone sweep (`internal/model`/`internal/caldav` peer writes, un-audited this pass). **Pass 21** (2026-07-25, `docs/audit/passes/PASS-21.md`) targeted the least-covered surfaces from the ledger (recurrence read-path re-fuzz, the go-ical heal-set spec-diff, CalDAV request-construction fault-injection, and the pass-20 reconcile/store classes' read-only and peer twins) — 3 confirmed findings (1 HIGH, 2 MED), all fixed repro-first, plus the 1 genuine canary escape closed. The HIGH was the **third reopening of the "heal set must mirror go-ical's full `validateComponent`" class** — this time at recursion depth: a phantom component nested under a VALARM/STANDARD/DAYLIGHT bricked the whole resource on re-encode; fixed by completing `model.allowedChildren` so `stripForbiddenNesting` strips it before go-ical's recursive `checkComponent` sees it, and the guardrail now demands matching that recursion depth. The MEDs: recurrence expansion had no *aggregate* step budget (N far-anchored high-frequency events froze each redraw — fixed with a shared `model.StepBudget`), and PROPPATCH treated any 207 as success without inspecting the per-property status (a server-rejected rename/recolor silently dropped — fixed by parsing the propstat status). Raw severity held (HIGH 1→1, total 5→3) but a HIGH data-brick plus the third reopening keep it `more_passes_recommended`; the cross-package resource-is-gone sweep remains the standing residual. **Pass 22** (2026-07-25, `docs/audit/passes/PASS-22.md`) took direct aim at that carried residual (`internal/model`/`internal/caldav` peer write paths, `data-loss`) and largely **retired** it: `internal/model` is pure/headless so the concurrent-delete class cannot manifest there by construction, and `internal/caldav` conditional-write paths held under fault injection (touched, no confirmed finding). The one confirmed finding was elsewhere — 1 MED (the v1.3.0 Custom repeat "Ends on date" dropped the selected end day for *timed* items, because UNTIL was stored at midnight and only made inclusive for all-day anchors) — fixed by anchoring UNTIL at the selected date + the anchor's own time-of-day. Findings converged (total 3→1, HIGH 1→0), but the mutation-canary net regressed sharply (3 of 4 escaped: caldav `normalizeETag` W/ strip, `drillRange` bounds, `reconcileReadOnly` dirty-discard) — all three closed with boundary tests verified to kill their mutations. `more_passes_recommended` stands (the test net, not the code, is the standing weak spot); the remaining `internal/caldav` write-path sweep is the residual. **Pass 23** (2026-07-25, `docs/audit/passes/PASS-23.md`) spent five of its six slots on ledger rows marked `never` — and they were high-yield, not low-yield: **15 confirmed findings (6 HIGH, 6 MED, 3 LOW)**, all fixed repro-first, plus the 1 escaped canary and 2 coverage holes closed. The HIGHs were a sidecar tombstone key that let sync **write outside the cache root**, a corrupt sidecar **discarding all sync metadata** (clobbering unsynced edits and resurrecting deletions), a **failed** conflict-resolve that still resolved in memory, the **fourth** reopening of the heal-set class, a `YEARLY;BYMONTHDAY` rule decoded as plain yearly (it fires 12×/year, and a grab made the event vanish), and a Θ(n²) `LayoutDay`. Two systemic lessons were codified as guardrails. First, the heal-set guardrail's "enumerate every container" wording was **unsatisfiable** — go-ical recurses into containers `checkComponent` has no case for — which is *why* that class kept reopening; the strip is now deny-by-default restricted to encoder-validated types, and `internal/model/encoderdrift_test.go` makes the gate enforce the table re-diff that had failed manually in three of the four reopenings. Second, **a fix can land, pass its test, and never reach production**: pass 21's `StepBudget` and pass 22's timed-UNTIL were both real fixes whose regression tests called the helper directly while the UI wiring never delivered the fixed value — now banned by a guardrail requiring a UI-reachable bug's guard to drive the real entry point. The arc also surfaced **four defects the audit missed**, the largest being that all-day "Ends on date D" *also* dropped D — two bugs whose signs flip with the UTC offset, **falsifying the pass-22 close-out** that recorded all-day as correct. Severity rose sharply (HIGH 0→6) as a **measurement effect**, not a regression: pass 22 re-examined thrice-hardened surfaces while pass 23 opened never-audited ones. `more_passes_recommended`; the carried residual is an **unverified product-bug lead** an agent reported before being cut off, plus the still-unswept `internal/caldav` write paths (second consecutive pass) and race/fault-injection (not exercised at all this pass).
