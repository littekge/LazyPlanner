...
The **full form** (`e` on an existing item, or the full-create key) edits every field **except the recurrence rule** — the forms still have no repeat field, so an existing rule can't be *rewritten* in-app (the scope pickers edit occurrences, never the RRULE itself); a full-form Repeat field is planned as v1.3.0. Parsing rules are predictable and documented in `:help` — when in doubt, text stays in the title rather than being guessed.
...
### v1.2.0 — quick-add parser improvements (implemented 2026-07-22)
...
**Status**: all six build steps implemented, each repro-first with a green full gate (see `log.md` for the per-step record). Verified headlessly — per-recognizer boundary tables, the adversarial zero-warning title table asserted verbatim, an extended `FuzzParseQuickAdd` (new-grammar seeds + a warning-only-fires-with-an-intent-anchor invariant, ~1M execs clean), and UI tests for the create paths and the keep-open re-prompt flow. Awaiting the owner's release/tag.
