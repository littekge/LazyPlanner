...
**Convergence is measured as a severity-weighted trend, not a raw finding count** (the explicit criteria live in `docs/audit/PROTOCOL.md` — "Convergence — the stop rule"). The trend is genuinely mixed: HIGH fell 5 (pass 10) → 1 (pass 13) → 0 (pass 14) → **2 (pass 15)**. Pass 15 was the gap-closing pass main.md had called for, and it proved the point — the never/stale matrix cells still harbored two HIGH silent-data-loss bugs at the CalDAV-write and store-load boundaries, not just a MED/LOW tail. The phase is **not converged**, and further from it than pass 14 suggested: (1) the headless matrix is closer to covered-once (pass 15 closed CalDAV response-parse, store disk-fault, and a direct store `-race`), but **mouse/`:config`** (stale since pass 10) remains; (2) criterion 2 (two consecutive no-HIGH passes) is **reset to zero** by pass 15's two HIGH; (3) no new root-cause class this pass. The lesson holds: finish covering the matrix (criterion 1) before trusting the severity trend. Permanently-accepted gaps: the **Raspberry Pi on real hardware** (unauditable headlessly); a UID-less component is display-only and a resource mixing it with a valid sibling drops the whole resource on import (pass-15 MED accepted residual — a fabricated UID would churn sync identity, and no fix avoids crossing a hard invariant); and two very-low-reachability go-ical encoder constraints (empty VCALENDAR, childless VTIMEZONE) are not auto-healed.
