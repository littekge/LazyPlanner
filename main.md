...
- **Pass 9** (2026-07-13) — pre-1.0 audit of the remaining unhardened areas: 16 findings fixed, incl. a path-traversal calendar id that could delete the whole account dir and three recurrence-edit data-loss/iron-rule bugs; added the `FuzzRecurrenceMutations` target.
...
The phase has **not converged** — each fresh method on a previously-skipped surface still finds real bugs — so readiness is treated as a trend across diverse passes, never a single clean verdict. Remaining targets: the **Raspberry Pi on real hardware** (the sole never-covered surface), the full **sync reconcile local×server case matrix** (pass 13 fixed one cell; the keep-both/`Forget` branches are not exhaustively enumerated), and re-sweeps of the stale **timezone/DST**, **UI draw-widget**, and **non-command input-edge** surfaces. Accepted known gaps: a UID-less component is display-only (a fabricated UID would churn sync identity), and two very-low-reachability go-ical encoder constraints (empty VCALENDAR, childless VTIMEZONE) are not auto-healed.
