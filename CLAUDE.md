...
- **UI**: lazygit-style — left column of focusable panels (Calendars/Tasks/Agenda), Main pane follows focus, right Detail pane, status bar. Task tree: full tree + `>`/`<` zoom. Creation: `a` quick-add with smart date/`!priority`/`#tag` parsing; `e` full form. See `main.md` UI Design for layout + keymap.
...
5. **Run the program** to verify it still builds and launches: `go build ./...` (and `go run .` for manual checks).
...
> This section will be expanded once the language and tooling are decided.
...
- **Linting**: `go vet ./...` and `staticcheck ./...` must pass after every code change. No other linters.
...
- **No magic numbers**: named constants for anything with meaning; user-facing tunables belong in the config file (format TBD).
