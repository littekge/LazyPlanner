...
- **`DUE` is the anchor for a recurring VTODO**, not just `DTSTART` for an event: a native recurring todo carries only `DUE`, which *is* its rule anchor. Every path that day-shifts a recurring todo's due — single grab `j`/`k`, bulk grab, the `sd` quick-set, any new one — must call `ReanchoredRecurrenceTodo(td, oldDue, newDue)` and block on `(nil,true)`.
- **Enumerate the doors, not the features.** Pass 24 found `sd` reaching the same anchor through `applyTodoField` while this guardrail's sweep list named only the two grab paths — the fix was real, tested, and simply not wired to every caller. The re-anchor therefore lives in **`applyTodoField`** (the quick-set chokepoint), not in `setDuePrompt`, so a future field mutation that shifts the due inherits it. When touching this class, grep for *every* writer of `TodoDraft.Due`, don't trust the list above.
- **Gate the re-anchor on the calendar day actually changing.** `reanchoredRecurrence` reports `blocked` for a rule outside the editable vocabulary whether or not the day moved, so an ungated call refuses a legitimate time-only set (`sd 3pm`) on a *Custom rule (kept)* todo with "can't shift the day" — a change that shifts no day. Both sides need a guard.
- Guards: `internal/model/reanchor_test.go`, `internal/ui/grab_recur_reanchor_test.go` (event), `internal/ui/grab_todo_reanchor_test.go` (single grab, todo), `internal/ui/bulkgrab_recur_reanchor_test.go` (bulk grab, todo), `internal/ui/sd_recur_reanchor_test.go` (quick-set, both sides of the day-change gate). Reopened once.
