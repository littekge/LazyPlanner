...
- **Every `tview.List` must set `SetSelectedStyle(selectionStyle)`.** The app sets `tview.Styles.PrimitiveBackgroundColor = tcell.ColorDefault` (`useTerminalTheme`), so tview's *default* List selected style — terminal-default foreground on a `PrimaryTextColor` (white) bar — renders illegibly (white-on-white) on a terminal-default background. The shared `selectionStyle` (`tcell.StyleDefault.Reverse(true)`) is theme-adaptive; every list uses it (`calendars`/`tasklists`/`agendaList`, the conflicts list, the `:account` picker). This class reappeared once (the v1.1.0 `:account` picker shipped without it). Regression tests assert the selected row is reverse-video: `TestSelectionIsLegible` and `TestAccountPickerSelectionIsLegible` (`internal/ui/{app,account}_test.go`) — add one for any new list.
