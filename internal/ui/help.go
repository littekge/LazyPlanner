...
var helpSections = []struct {
	...
	{"Edit & organize", [][2]string{
		...
		{"e / d on recurring event", "prompts scope: this occurrence / this & future / all"},
		{"e / d on recurring task", "prompts scope: this task / all tasks (no this & future — a task has one live instance)"},
	...
	{"Sync & commands", [][2]string{
		...
		{":calendar", "new / rename / color / hide / show (`new` opens the create form; `color` with no hex opens the swatch picker); `rename`/`color`/`hide`/`show` target the focused task list from the Tasks pane"},
