...
- **Mode indicator**: the status bar (now outlined like the other panes) shows a vim-style **mode badge** at its far left — `NORMAL` at rest, `DRILL` when you've dived into a calendar day or task tree, and `GRAB` in grab mode. It tells you what the movement keys (`hjkl`) act on right now, so a context-sensitive key is never a surprise.
