...
- **`H` / `L`** — outdent / indent the selected task (re-parent). **`y`** cuts (move) a task and its subtree, **`Y`** copies it; **`p`** pastes under the selected task and **`P`** at the list's top level. A cut moves it (same list = re-parent, other list = move to that calendar); a copy duplicates it with fresh UIDs, leaving the original. The clipboard **persists after a paste**, so you can paste the same item multiple times. **`u`** — undo the last change this session (multi-level).
